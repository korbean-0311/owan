...
- **개인 누적 벌금 상한 10만원**.
- **레버리지**: 시즌당 **3회**. 이번 주를 다음 주와 합쳐 **2×목표**로 판정. 마지막 주차 불가. **실패 시 두 주 벌금 합의 2배** 부과.
...
- 데이터 보존 변경: 별도 마이그레이션 SQL 작성(ALTER/CREATE OR REPLACE/UPDATE). 예: `supabase/migration_colors.sql`, `migration_colors2.sql`, `migration_storage_delete.sql`(certs 버킷 DELETE 정책 — 본인/방장만, 사진 파일 정리용), `migration_leverage3.sql`(레버리지 2→3, 전체 초기화 시엔 schema.sql에 이미 포함).
