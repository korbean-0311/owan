...
- **승인왕 순위**: 남의 인증에 "승인" 많이 눌러준 순위(`certification_approvals.approver_id` 집계, `loadApprovalRanking`). **절반 주차(`ceil(N/2)`)에만** 홈에 카드 노출(Top3+Bottom3+본인 + 피드 유도 버튼, `renderApprovalCard`). 시즌 종료 정산 화면엔 전원 표시. 승인 기록은 시즌 리셋 시 삭제(시즌 단위).
