...
- **주차 마감 = 월요일 정오 12시**: `WEEK_CLOSE_MS=12h`. `computeCurrentWeek()`와 `inWeek()`는 각 주차를 다음 월요일 12:00까지 열어둔다. 일요일 밤 인증이 월요일 오전에 승인돼도 지난 주차로 집계되도록 하기 위함. (집계는 여전히 제출 시간/승인 상태 기준)
