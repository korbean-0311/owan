...
- **승인왕 순위**: 남의 인증에 "승인" 많이 눌러준 순위(`certification_approvals.approver_id` 집계, `loadApprovalRanking`). **3주마다(3·6·9·12…주차)** 홈에 카드 노출(`isApprovalWeek`: `CURRENT_WEEK%3===0`, 그 한 주만)(Top3+Bottom3+본인 + 피드 유도 버튼, `renderApprovalCard`). 시즌 종료 정산 화면엔 전원 표시. 승인 기록은 시즌 리셋 시 삭제(시즌 단위).
...
## 11. 주요 한계/주의 (gotcha)
- **모바일 웹은 카메라 사진의 EXIF를 제거** → 촬영일 자동검증 불가. 그래서 "사진 찍기(camera 강제)=직접촬영 신뢰배지". "사진 선택(갤러리)"은 **EXIF 카메라 태그(Make/Model/DateTimeOriginal)로 판별** → 있으면 `photo`(✓ 카메라 사진, 촬영일로 주차검증), 없으면 `capture`(📷 화면 캡쳐·멤버 확인). ⚠️ `CreateDate`/`ModifyDate`는 스샷에도 붙어서 안 씀(과거 버그). 진짜 검증은 2명 승인.
